...
let type, folder, prefix, label, name, js, jsinit, outputDir;
        ...
        try {
            inquirer.prompt([
                ...
                if (type === 'component') {
                    ...
                    folder = '/03-modules/' + res.moduletype + '/';
                    prefix = 'mod-';
                ...
                if (!fs.existsSync(fractal.components.get('path') + folder)) {
                    fs.mkdirSync(fractal.components.get('path') + folder, '0777');
                }
                // create mod/cmp directory
                if (!fs.existsSync(fractal.components.get('path')+folder+name)){
                    fs.mkdirSync(fractal.components.get('path')+folder+name, '0777');
                } else {
                    reject(folder + name + ' already exists !!!');
                }
    
                outputDir = fractal.components.get('path')+folder+name+'/'
                resolve()
            })
        } catch (error) {
            reject(error)
