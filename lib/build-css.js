 ...
 */
// only global vars
const globalVars =  fractal.components.get('path') + '/00-globals/00-glb-vars/glb-vars.scss';
// only globals without vars
const globals = fractal.components.get('path') + '/00-globals/!(00-glb-vars)/*.scss';
// all scss files without globals, styleguide or critical
const components = fractal.components.get('path') + '/!(00-globals)/**/!(*.critical|*.styleguide)*.scss';
// only styleguide files
const styleguide = fractal.components.get('path') + '/**/*.styleguide.scss'

const outputDir = args.release ? fractal.get('project.release') + '/release/assets/css/' : fractal.web.get('static.path') + '/assets/css/';
...
async function concatScss(files) {
    try {
        return await concat(files).then(result => {
            console.log('merged all scss files', files)
            return result
        })
    } catch (err) {
        console.error(err)
...
function convertToCss(file) {
    return new Promise(function (resolve, reject) {
        try {
            sass.render({ data: file, sourceMap: true, outFile: outputDir + outputFileName + '.css' }, function (err, result) {
