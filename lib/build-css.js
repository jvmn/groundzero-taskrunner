...
const globals = `${ projectPath.dev }/00-globals/!(00-glb-vars)/!(*.glb)*.scss`
// all scss files without globals, styleguide or critical
const components = `${ projectPath.dev }/!(00-globals)/**/!(*.critical|*.styleguide|*.glb)*.scss`
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      sass.render({ includePaths: [process.env.PROJECT_CWD + '/develop/'], data: file, sourceMap: true, outFile: outputDir + outputFileName + '.css' }, function (err, result) {
        ...
        if (err) console.error('error in sass to css conversion:', err)
        // return result
        resolve(result)
      })
    } catch (e) {
      ...
      fs.readFile(outputPath + '.css', (err, css) => {
        postcss([autoprefixer])
          ...
          .then(result => {
            ...
            const versionString = '/* Version: '+packageJson.version+', Build date: '+ new Date().toISOString() + ' */\n\n'
            fs.writeFile(outputPath + '.css', versionString + result.css, () => resolve())
            if (result.map) {
